...
{	
	...
	// 创建一个HttpHeaderCursor对象，用于遍历请求的HTTP头部
	...
	resp->append_output_body_nocopy("<html>", 6); 					// 在响应正文的开始和结束位置分别添加"<html>"和"</html>"
	len = snprintf(buf, 8192, "<p>%s %s %s</p>", req->get_method(), // 向响应正文中添加请求的HTTP方法、请求URI和HTTP版本信息
				   req->get_request_uri(), req->get_http_version());
	resp->append_output_body(buf, len);

	// 遍历请求的所有HTTP头部，将头部的名称和值添加到响应正文中
...
static WFFacilities::WaitGroup wait_group(1); // 等待的任务数量为1
