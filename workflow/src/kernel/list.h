 ...
 * @note:   (type *)0：将一个空指针强制转换为结构体类型 type 的指针，这是为了获取到结构体的起始地址。
 */
 ...
 * @pos:	the &struct list_head to use as a loop counter.
 * @n:		another &struct list_head to use as temporary storage 一个临时变量，用于保存下一个节点的指针。在遍历过程中，可能会删除或添加节点，所以需要使用临时变量来保存下一个节点，以确保遍历的安全性
 * @head:	the head for your list.
 */
